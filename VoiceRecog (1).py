...
class Ui_MainWindow(object):
    ...
    def record_audio3(self):
        while self.recording3:
            ...
            #myrecording = np.append(myrecording,myrecording)
            ...
            '''
    def record_audiio4(self):
        self.audio = pyaudio.PyAudio()
        self.stream = self.audio.open(format=au_format, channels=no_channels,rate=sampling_rate, input=True, frames_per_buffer=chunk)
        self.frames = []
        for i in range(0, int(sampling_rate / 1024 * 3)):
            data = self.stream.read(chunk)
            self.frames.append(data)
        self.stop_recording4()
        '''

    def detect_stream(self,filename):
        '''
        ...
        '''
        name = obj2.conversation_mode(sampling_rate,filename)
        if name == 'unknown':
            self.display3()
        else:
            self.display2(name)
    ...
    def stop_recording4(self):
        try:
            self.stream.stop_stream()
            self.stream.close()
            self.audio.terminate()
            self.file_name = 'converse_'+str(datetime.datetime.now()).replace(" ", "_").replace(":", "_").replace(".", "_")+'.wav'
            path_name = os.getcwd()
            self.filepath2 = [""]
            self.filepath2[0] = path_name+"/"+self.file_name
            self.waveFile = wave.open(self.file_name, 'wb')
            ...
            self.waveFile.close()
            self.stream = None
            self.detect_stream(self.filepath2[0])
        except Exception as e:
            print('Stream closed')
    ...
    def start_training(self):
        ...
        '''
        song = AudioSegment.from_wav(self.filepath[0])
        song= song + 5
        song.export(self.filepath[0],format="wav")
        '''
        ...
        dest=self.output_file('./noise_cleaned_samples/',name+'_noise_cleaned_simple.wav', y_reduced_centroid_s, sr, '_ctr_s')
        ...
        try:
            ...
            if(name == None):
                ...
                #sql = "UPDATE USERS SET REP = ? WHERE ID = ? "#add quotes
        ...
        #print ('Done...')
        '''
        register_obj.reg_train(self.filepath[0], name)
        #register_obj.reg_train('./noise_cleaned_samples/'+name+'_noise_cleaned_simple_ctr_s.wav', name)
        #self.showdialog('Information updated.','Provides fields has been saved.')
    ...
    def state_changed(self):
        ...
        #try:
            if(self.checkBox.isChecked()):
                ...
                #self.background_thread.start()
                claim_name = self.identity_field.text()
                ...
                if(claim_name == None):
                    ...
                    '''
                    song = AudioSegment.from_wav(self.filepath2[0])
                    song= song + 5
                    song.export(self.filepath2[0],format="wav")
                    '''
                    ...
                    y_reduced_centroid_s, time_trimmed = self.trim_silence(y_reduced_centroid_s)
                    print ("Silent seconds trimmed: " + str(time_trimmed))
                    dest = self.output_file('./noise_cleaned_samples/',claim_name+'_noise_cleaned_simple.wav', y_reduced_centroid_s, sr, '_ctr_s')
                    ...
                    (rate,sig)=wav.read(dest)
                    mfcc_feat = mfcc(sig,rate, winlen=0.001, winstep=0.01,nfft=512, numcep = 25)
                    '''
                    name = obj2.conversation_mode(sampling_rate,sig)
                    self.display(name)
                    print (name)
                    '''
                    for row in cursor:
                        recognize_obj = nm.recognition(row[0])
                        score = recognize_obj.recog(mfcc_feat)
                        print("after identity: {}".format(score))
                        #if identity == True:
                            #self.display(claim_name)
                       # else:
                            #self.label_18.setPixmap(QtGui.QPixmap("chat3.png"))
    ...
    def trim_silence(self,y):
        y_trimmed, index = librosa.effects.trim(y, top_db=10, frame_length=2, hop_length=500)
